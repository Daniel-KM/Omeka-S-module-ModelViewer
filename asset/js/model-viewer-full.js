...
document.addEventListener('DOMContentLoaded', function(event) {
    ...
    function createScene(options) {
        ...
        //     ? options.config.import.loaders.includes('Addons.EXRLoader')
        //     : false;
        ...
        if (!options.mediaType
            ...
            const loader = new Addons.GLTFLoader(manager);
            loader
                .load(
                    modelSource,
                    gltf => {
            ...
            manager
                .addHandler(/\.dds$/i, new Addons.DDSLoader());
            if (options.mtl && options.mtl.length) {
                new Addons.MTLLoader(manager)
                    .load(options.mtl[0], function (materials) {
                        materials.preload();
                        new Addons.OBJLoader(manager)
            ...
            if (options.mtl && options.mtl.length) {
                new Addons.MTLLoader(manager)
                    .load(
                        ...
                        function (materials) {
                            materials.preload();
                            new Addons.ColladaLoader(manager)
                                .load(
                                    options.source,
                                    function (collada) {
                ...
                const loader = new Addons.ColladaLoader(manager);
                loader
                    .load(
                        options.source,
                        collada => {
            ...
            const loader = new Addons.VRMLoader();
            loader
                .load(
                    options.source,
                    function (obj) {
        ...
        function enableOrbitControls() {
            ...
            control = new Addons.OrbitControls(camera, canvas);
