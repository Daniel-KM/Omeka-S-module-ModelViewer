...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function createScene(options) {
        ...
        const control = new Addons.OrbitControls(camera, canvas);
    ...
    function loadModel(options, scene, manager, modelScale, matcap, anisotropy, onLoaded) {
        ...
        if (mediaType === 'model/gltf-binary'
            ...
            new Addons.GLTFLoader(manager).load(options.source, function (gltf) {
            ...
            manager.addHandler(/\.dds$/i, new Addons.DDSLoader());
            if (options.mtl && options.mtl.length) {
                new Addons.MTLLoader(manager).load(options.mtl[0], function (materials) {
                    materials.preload();
                    new Addons.OBJLoader(manager).setMaterials(materials).load(options.source, function (object) {
                        scene.add(object);
                        onLoaded(object);
                    });
                });
            } else {
                new Addons.OBJLoader(manager).load(options.source, function (object) {
                    scene.add(object);
                    onLoaded(object);
                });
            }
            ...
            if (options.mtl && options.mtl.length) {
                new Addons.MTLLoader(manager).load(options.mtl[0], function (materials) {
                    materials.preload();
                    new Addons.ColladaLoader(manager).load(options.source, function (collada) {
            ...
            new Addons.FBXLoader().load(options.source, function (object) {
    ...
    function setupEnvironment(scene, renderer) {
        ...
        var roomEnv = new Addons.RoomEnvironment(renderer);
